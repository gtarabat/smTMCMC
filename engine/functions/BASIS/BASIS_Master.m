...
% Last update: 2016 Feb 12, by Daniel Waelchli
%   2016-02-12: (Daniel)
%       1. included rescaling option for Sigma
%   2016-02-08: (Daniel)
%       1. N_accept does not include burn-in
%       2. added N_correct, N_posdef
%   2016-02-02: (Daniel)
%       1. parameter to turn-on/off parallelization
%   2015-05-04:
...
if sys_para.enable_para
	start_parallel( sys_para.N_cpu, sys_para.spmd_par );
end
addpath(genpath(sys_para.model_folder))
...
runinfo.N_accept = []; % initialize number accepted samples for stage 1
runinfo.N_posdef = []; % initialize number posdef proposals for stage 1
runinfo.N_correct = []; % initialize corrections on Sigma for stage 1

%% Restart
if sys_para.restart
    runinfo = continue_BASIS(sys_para);
    sys_para = runinfo.sys_para; % retrieve sys_para
else
    runinfo.sys_para = sys_para; % store sys_para 
...
while runinfo.gen < sys_para.max_gen
    ...
    if sys_para.TF_msg
        fprintf('%s \n', sprintf('%s',repmat('-',100,1)) )
        
        if runinfo.gen > 1
            fprintf('Master: Stage %d completed (p = %.2e)\n',runinfo.gen,runinfo.p(runinfo.gen));
            fprintf('%.2f%% Acceptance rate\n',  runinfo.N_accept(end)/length(runinfo.accept)*100 );
            fprintf('%.2f%% Positive definite Hessians\n',  runinfo.N_posdef(end)/length(runinfo.posdef)*100 );
            fprintf('%.2f%% Correction on Sigma\n', runinfo.N_correct(end)/length(runinfo.correction_sigma)*100 );
            fprintf('%s \n', sprintf('%s',repmat('-',100,1)) )
        end
...
if runinfo.gen == 1
    ...
    for i = 1:sys_para.N_cpu
        ...
        work(i).eps = runinfo.eps(job_ind{i},:);
    end
...
if sys_para.enable_para && sys_para.N_cpu > 1 && ~sys_para.spmd_par 
    % PARFOR parallelization
    parfor i = 1:sys_para.N_cpu
        out_worker{i} = BASIS_Worker(i,work(i),sys_para,local_info);
    end
elseif sys_para.enable_para && sys_para.N_cpu > 1
    spmd
...
runinfo.out_gradient = zeros(tmp,sys_para.N_dim);
runinfo.out_gamma = zeros(tmp,sys_para.N_dim);
runinfo.Ns = zeros(tmp,1);
runinfo.pri = zeros(tmp,1);
runinfo.lik = zeros(tmp,1);
runinfo.out_lik = cell(tmp,1);
runinfo.out_sig = cell(tmp,1);
runinfo.eps = zeros(tmp,1);
runinfo.burnin_reps = zeros(tmp,1);
runinfo.accept = zeros(tmp,1);
runinfo.bds_flg = zeros(tmp,sys_para.N_dim);
runinfo.posdef = zeros(tmp,1);
runinfo.correction_sigma = zeros(tmp,1);
runinfo.N_accept(end+1) = 0;
runinfo.N_posdef(end+1) = 0;
runinfo.N_correct(end+1) = 0;
...
for i = 1:sys_para.N_cpu
    for j = 1:length(out_worker{i})     
        ...
        runinfo.out_gradient(tmp+1:tmp+len_out,:) = out_worker{i}(j).out_gradient;
        runinfo.out_sig(tmp+1:tmp+len_out) = out_worker{i}(j).out_sig;
        runinfo.out_gamma(tmp+1:tmp+len_out,:) = out_worker{i}(j).out_gamma;
        runinfo.eps(tmp+1:tmp+len_out) = out_worker{i}(j).eps;
        runinfo.burnin_reps(tmp+1:tmp+len_out) = out_worker{i}(j).burnin_reps;
        runinfo.accept(tmp+1:tmp+len_out) = out_worker{i}(j).accept;
        runinfo.bds_flg(tmp+1:tmp+len_out,:) = out_worker{i}(j).bds_flg;
        runinfo.posdef(tmp+1:tmp+len_out) = out_worker{i}(j).posdef;
        runinfo.correction_sigma(tmp+1:tmp+len_out) = out_worker{i}(j).correction_sigma;
        tmp = tmp + len_out;
    end
   
end
runinfo.N_accept(end) = sum(runinfo.accept);
runinfo.N_posdef(end) = sum(runinfo.posdef);
runinfo.N_correct(end) = sum(runinfo.correction_sigma);
end
